...
export function AuthProvider({ children }) {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (token) {
        const response = await fetch("http://192.168.1.5:5000/auth/user", {
          method: "GET",
          headers: {
  ...
  const logout = async () => {
    const token = await AsyncStorage.getItem("token");
    const response = await fetch("http://192.168.1.5:5000/auth/logout", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const signup = async (name, email, password) => {
    try {
      const response = await fetch("http://192.168.1.5:5000/auth/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
